    ...
    public class TeamUnitToPathSystem : SystemBase
    {
        ...
        public void Construct()
        {
            ...
            FlowFieldChangeStream
                .Subscribe(teamTileContext =>
                {
                    ...
                    //
                    UpdateTeamAtiTiles(teamTileContext);
        ...
        protected override void OnUpdate()
        {
            ...
            Entities
                ...
                .ForEach((Entity entity, FlowFieldTileChangeInfo flowFieldTileChangeInfo) =>
                {
                    var teamId = flowFieldTileChangeInfo.TeamId;
                    
                    ResetCachedFlowFieldEntities(commandBuffer, teamId);
                    
                    _notifyTileChange.OnNext(new TeamTileContext
                    {
                        TeamId = flowFieldTileChangeInfo.TeamId,
                        TimeTick = flowFieldTileChangeInfo.TimeTick,
                        TargetPosition = flowFieldTileChangeInfo.TargetPosition
                    });
        ...
        private void ResetCachedFlowFieldEntities(EntityCommandBuffer commandBuffer, int teamId)
        {
            ...
            foreach (var pair in table)
            {
                ...
                commandBuffer.AddComponent(entity, new GameEnvironment.DiscardedFlowFieldTile());
            }
            
            table.Clear();
        }

        private void UpdateTeamAtiTiles(TeamTileContext teamTileContext)
        {
            //
            ...
            Entities
                ...
                {
                    var teamId = teamForce.TeamId;
                    // _logger.Debug($"TeamUnitToPathSystem - UpdateTeamAtiTiles - teamId: {teamId} inTeamId {inTeamId}");
                    if (teamId == teamTileContext.TeamId)
                    {
                        var tileIndex = GetTileIndex(translation.Value);
                        
                        var existed = _teamAtTiles[teamId].TileIndices.Exists(x => x == tileIndex);
                        if (!existed)
                        {
                            _teamAtTiles[teamId].TileIndices.Add(tileIndex);
                        }                            
                    }
                })
                .WithoutBurst()
            ...
            List<Vector3> path)
        {
            ...
            {
                var tileIndex =
                    GameEnvironment.Utility.PathTileHelper.TileIndex1d(
            ...
            Vector3 targetPosition, List<int> tileIndices)
        {
            ...
            var entityArchetype = entityManager.CreateArchetype(
                typeof(GameEnvironment.FlowFieldTile),
                typeof(GameEnvironment.FlowFieldTileCellBuffer),
                typeof(GameEnvironment.FlowFieldTileInCellBuffer),
                typeof(GameEnvironment.FlowFieldTileOutCellBuffer));
            ...
            for (var i = 0; i < flowFieldEntities.Count; ++i)
            {
                if (i != flowFieldEntities.Count - 1)
                {
                    ...
                    entityManager.SetComponentData(flowFieldEntity, new GameEnvironment.FlowFieldTile
                    {
                        HorizontalCount = 10,
                        VerticalCount = 10,
                        TimeTick = timeTick,
        ...
        private static void SetupFlowFieldTileEntity(EntityManager entityManager, int teamId, int timeTick, Vector3 targetPosition, Dictionary<int, Entity> cachedEntities, int tileIndex, Entity entity)
        {
            ...
            entityManager.SetComponentData(entity, new GameEnvironment.FlowFieldTile
            {
                Index = tileIndex,
            ...
            var tileBuffer = entityManager.AddBuffer<GameEnvironment.FlowFieldTileCellBuffer>(entity);
            var tileCellInBuffer = entityManager.AddBuffer<GameEnvironment.FlowFieldTileInCellBuffer>(entity);
            var tileCellOutBuffer = entityManager.AddBuffer<GameEnvironment.FlowFieldTileOutCellBuffer>(entity);
