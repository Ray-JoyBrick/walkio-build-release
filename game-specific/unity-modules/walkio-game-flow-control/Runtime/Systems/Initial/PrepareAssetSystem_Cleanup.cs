namespace JoyBrick.Walkio.Game.FlowControl
{
        ...
        // GameCommon.ISystemContext
    {
        ...
        {
            _logger.Debug($"Module - Flow Control - PrepareAssetSystem - RegisterToCleanupFlow");

            FlowControl?.AssetUnloadingStarted
                .Where(x => x.Name.Contains(AtPart))
                .Subscribe(x =>
                {
                    _logger.Debug($"Module - Flow Control - PrepareAssetSystem - RegisterToCleanupFlow - Receive AssetLoadingStarted");
