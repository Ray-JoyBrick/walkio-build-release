namespace JoyBrick.Walkio.Game.Hud.App
{
        ...
        // GameCommon.ISystemContext
    {
        ...
        private void ActivateLoadingView(bool flag)
        {
            ...
            if (flag)
            {
                ExtensionService?.SendExtensionEvent("zz_Activate_Loading_View");
                // GameExtension.BridgeExtension.SendEvent("zz_Activate_Loading_View");
            }
            else
            {
                ExtensionService?.SendExtensionEvent("zz_Deactivate_Loading_View");
        ...
        public void Construct()
        {
            ...
            CommandService?.CommandStream
                // .Do(x => _logger.Debug($"Module - Hud {AtPart} - PrepareAssetSystem - Construct - Receive Command Stream: {x}"))
                .Where(x => (x as GameCommand.ActivateLoadingViewCommand) != null)
                .Subscribe(x =>
                {
                    _logger.Debug($"Module - Hud {AtPart} - PrepareAssetSystem - Construct - Receive ActivateLoadingViewCommand");
