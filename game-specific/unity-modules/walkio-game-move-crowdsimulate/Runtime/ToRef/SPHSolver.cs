namespace JoyBrick.Walkio.Game.Move.CrowdSimulate
{
    ...
    {
        private readonly List<ParticleProperty> _particleProperties = new List<ParticleProperty>();
        ...
        private int CurrentParticleCount => _particleProperties.Count;
            ...
            ComputeShader shaderSPH)
        {
            ...
            _kernelCSMain = _shaderSPH.FindKernel("CSMain");

            //
            _shaderSPH.SetFloat("_Stiffness", _stiffness);
        }

        public void AddParticles(IEnumerable<ParticleProperty> particleProperties)
        {
            _particleProperties.Clear();
            _particleProperties.AddRange(particleProperties);
        }

        public void InitializeOnGPU()
        {
            _shaderSPH.SetInt("_ParticleCount", CurrentParticleCount);
        }

        public void StepOnGPU()
        {
            _bufferParticles = new ComputeBuffer(CurrentParticleCount, ParticleProperty.Stride);
            _bufferParticles.SetData(_particleProperties);

            _shaderSPH.SetBuffer(_kernelCSMain, "_Particles", _bufferParticles);
